"""Offline API + real local FFmpeg verification, invoked by verify.sh."""
import os
if os.environ.get('AURALIS_TEST_OFFLINE')!='1' or not os.environ.get('AURALIS_CONFIG_DIR'):
    raise SystemExit('Use scripts/verify.sh with isolated offline configuration')
...
with TestClient(app) as client:
    ...
    assert health['application']=='auralis' and health['ready'], health
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    line = SimpleNamespace(id=7, chapter_id=3, audio_path=target_stub, should_speak=0, track="sfx")
    repo = FakeRepo(line)
    ...
    placeholder_line = SimpleNamespace(id=8, chapter_id=3, audio_path=target_stub, should_speak=0, track="sfx", production_note="[AURALIS_PLACEHOLDER_MATERIAL] draft")
...
with tempfile.TemporaryDirectory() as tmp:
    ...
    processed=service.resolve_audio_path(lines[1])
    if processed==lines[1].audio_path or not processed.endswith('.wav') or not os.path.isfile(processed):
        raise SystemExit(f"non-wav process did not create an independent selected WAV: {processed}")
    if not os.path.isfile(lines[1].audio_path):
        raise SystemExit('legacy processing removed the original source')
