...
def get_line_audio(
    ...
    media_type = _detect_audio_media_type(audio_path)
    return FileResponse(
        audio_path,
...
def attach_audio_asset(
        line_id: int,
        dto: LineAssetAttachDTO,
        line_service: LineService = Depends(get_line_service),
):
    try:
        target_path = line_service.attach_audio_asset(line_id, dto.source_path)
        return Res(
            data={"line_id": line_id, "audio_path": target_path},
            code=200,
            message="素材音频已绑定",
        )
    except (ValueError, FileNotFoundError) as exc:
        ...
        logging.exception("[attach_audio_asset] 失败")
        raise HTTPException(status_code=500, detail=f"绑定素材音频失败: {exc}") from exc
...
async def generate_audio(
    ...
    line = db.get(LinePO, dto.id) if dto.id else None
    if dto.id and not line:
        raise HTTPException(status_code=404, detail="台词不存在")
    if line and line.chapter_id != chapter_id:
        raise HTTPException(status_code=409, detail="台词不属于目标章节")

    if dto.should_speak == 0 or dto.track in {"sfx", "bgm"} or dto.line_type in {"sfx", "bgm"}:
        task = AudioTaskService(db).create_skipped(project_id, chapter_id, line, session_id) if line else None
        await manager.broadcast({
            ...
            "line_id": dto.id,
            "status": "skipped",
            "progress": request.app.state.tts_queue.qsize(),
            "meta": "素材轨不进入 TTS，请导入或制作音效/BGM 文件"
        })
        return {
            "code": 200,
            "message": "素材轨已跳过 TTS，请导入或制作音效/BGM 文件",
            "data": {"line_id": dto.id, "task_id": task.id if task else None, "skipped": True}
        }

    if not line:
        raise HTTPException(status_code=400, detail="生成音频需要有效的台词 ID")
    q = request.app.state.tts_queue
    try:
        task = AudioTaskService(db).enqueue(q, project_id, chapter_id, line, dto, session_id)
    ...
    return {"code": 200, "message": "已入队", "data": {"line_id": dto.id, "task_id": task.id, "queue_size": queue_size}}
