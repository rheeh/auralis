...
from app.models.po import LinePO, TTSProviderPO, VoicePO
...
async def add_smart_role_and_voice(project_id: int,chapter_id: int,
    ...
    # 首先获取项目下所有角色
    roles = role_service.get_all_roles(project_id)
#     只匹配实际需要朗读的角色；每次自动匹配会重新分配，保证全体角色音色唯一。
    speakable_role_ids = set(db.execute(
        select(LinePO.role_id).where(
            LinePO.chapter_id == chapter_id,
            LinePO.should_speak == 1,
            LinePO.track.notin_(["sfx", "bgm"]),
        )
    ).scalars().all())
    speakable_roles = [role for role in roles if role.id in speakable_role_ids and role.name not in {"音效", "BGM"}]
    role_names = [role.name for role in speakable_roles]
    # 获取所有音色
    voices = db.execute(
        select(VoicePO)
        .join(TTSProviderPO, TTSProviderPO.id == VoicePO.tts_provider_id)
        .where(TTSProviderPO.status == 1)
        .order_by(TTSProviderPO.name.asc(), VoicePO.name.asc())
    ).scalars().all()
    if len(voices) < len(role_names):
        return Res(
            data=None,
            code=409,
            message=f"可用音色只有 {len(voices)} 个，但需要为 {len(role_names)} 个角色分配不同音色。请先补充音色库。",
        )
    # 只要音色的名字和描述
    voice_names = [
        {
