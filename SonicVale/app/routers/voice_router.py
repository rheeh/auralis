...
@router.post("/presets/edge", response_model=Res[VoicePresetSeedResultDTO],
             summary="生成常见中文音色",
             description="使用 Edge-TTS 自动生成一组常见中文声线样例，并写入指定 TTS 引擎的音色库")
def seed_edge_voice_presets(
    ...
    try:
        ...
        success_count, skipped_count, failed_count, skipped_names, failed_names, target_dir = voice_service.seed_common_edge_voices(
            dto.tts_provider_id,
            target_dir=dto.target_dir,
            overwrite=bool(dto.overwrite),
        )
        result = VoicePresetSeedResultDTO(
            success_count=success_count,
            skipped_count=skipped_count,
        ...
        if failed_count and not success_count:
            return Res(data=result, code=500, message="常见音色生成失败，请检查网络或 Edge-TTS 可用性")
        return Res(data=result, code=200, message=f"生成完成：新增/更新{success_count}个，跳过{skipped_count}个，失败{failed_count}个")
    except Exception as e:
        return Res(data=None, code=500, message=f"生成常见音色失败: {str(e)}")
...
def seed_cosyvoice_presets(
    ...
    try:
        ...
        if tts_provider is None:
            return Res(data=None, code=400, message=f"tts服务提供商 '{dto.tts_provider_id}' 不存在")
        result_data = voice_service.seed_common_cosyvoice_voices(
            tts_provider,
            target_dir=dto.target_dir,
            overwrite=bool(dto.overwrite),
        )
        success_count, skipped_count, failed_count, skipped_names, failed_names, target_dir = result_data
        result = VoicePresetSeedResultDTO(
            success_count=success_count, skipped_count=skipped_count, failed_count=failed_count,
            skipped_names=skipped_names, failed_names=failed_names, target_dir=target_dir,
        )
        if failed_count and not success_count:
            return Res(data=result, code=500, message="CosyVoice 预置音色生成失败，请检查 API Key、地域和模型配置")
        return Res(data=result, code=200, message=f"生成完成：新增/更新{success_count}个，跳过{skipped_count}个，失败{failed_count}个")
    except ValueError as e:
        return Res(data=None, code=400, message=str(e))
    except Exception as e:
        return Res(data=None, code=500, message=f"生成 CosyVoice 预置音色失败: {str(e)}")
...
@router.get("/{voice_id}/audio", summary="播放音色参考音频")
def get_voice_audio(voice_id: int, voice_service: VoiceService = Depends(get_voice_service)):
    entity = voice_service.get_voice(voice_id)
    if not entity:
        raise HTTPException(status_code=404, detail="音色不存在")
    if not entity.reference_path:
        raise HTTPException(status_code=404, detail="该音色未设置参考音频")

    audio_path = os.path.abspath(os.path.expanduser(entity.reference_path))
    if not os.path.isfile(audio_path):
        raise HTTPException(status_code=404, detail=f"音频文件不存在: {entity.reference_path}")
