...
from app.core.prompts import get_context2lines_prompt, get_add_smart_role_and_voice, get_audio_drama_adaptation_rules
...
class ChapterService:
    ...
    def fill_prompt(self,template: str, characters: list[str], emotions: list[str], strengths: list[str],
        ...
        # 即使项目仍绑定旧的数据库提示词，也强制注入最新的广播剧声音优先规范。
        result = f"{get_audio_drama_adaptation_rules()}\n\n{template}"
    ...
    def add_smart_role_and_voice(self,project,content, role_names, voice_names):
        ...
        try:
            ...
            if not isinstance(parse_data, list):
                raise ValueError("智能匹配没有返回角色音色列表")
            # 音色来源可以跨 TTS Provider；ID 已由调用方按启用供应商过滤。
            voice_id_map = {voice["name"]: voice["id"] for voice in voice_names}
            if len(voice_id_map) < len(role_names):
                raise ValueError("可用音色数量少于角色数量，无法保证每个角色使用不同音色")

            requested = {
                str(item.get("role_name") or "").strip(): str(item.get("voice_name") or "").strip()
                for item in parse_data if isinstance(item, dict)
            }
            assignments = self._unique_voice_assignments(role_names, list(voice_id_map), requested)
    ...
    @staticmethod
    def _unique_voice_assignments(role_names, voice_names, requested):
        if len(voice_names) < len(role_names):
            raise ValueError("可用音色数量少于角色数量，无法保证每个角色使用不同音色")
        unused_voice_names = list(dict.fromkeys(voice_names))
        assignments = []
        for role_name in role_names:
            voice_name = requested.get(role_name)
            if voice_name not in unused_voice_names:
                voice_name = unused_voice_names[0] if unused_voice_names else None
            if not voice_name:
                raise ValueError(f"角色 {role_name} 没有可用的独立音色")
