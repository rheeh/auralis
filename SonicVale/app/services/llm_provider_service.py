...
class LLMProviderService:
    ...
    def create_llm_provider(self,  entity: LLMProviderEntity):
        ...
        snapshot_provider_config("llm", "after_create", res)
    ...
    def update_llm_provider(self, llm_provider_id: int, data:dict) -> bool:
        ...
        if existing and existing.id != llm_provider_id:
            return False
        snapshot_provider_config("llm", "before_update", po)
        keep_when_empty = {"api_base_url", "api_key", "model_list", "custom_params"}
        data = {
            key: value
            for key, value in data.items()
            if not (key in keep_when_empty and value == "")
        }
        updated = self.repository.update(llm_provider_id, data)
        snapshot_provider_config("llm", "after_update", updated)
        return True
    ...
    def delete_llm_provider(self, llm_provider_id: int) -> bool:
        ...
        """
        po = self.repository.get_by_id(llm_provider_id)
        snapshot_provider_config("llm", "before_delete", po)
    ...
    def test_llm_provider(self, entity: LLMProviderEntity):
        ...
        if entity.api_base_url is None or entity.api_key is None or entity.model_list is None:
            return False, "LLM Base URL、API Key 和模型名不能为空", None
        ...
        if res is None:
            return False, "LLM 未返回任何内容", None

        # 7. 校验返回是否为合法 JSON
        try:
            # res = res.replace("```json",'')
            # res = res.replace("```",'')
            sample_output = json.loads(res)
        except json.JSONDecodeError:
            return False, "LLM 返回的内容不是合法 JSON，请检查模型 / 提示词", None
        return True, "测试成功", {"sample_output": sample_output}
