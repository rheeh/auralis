...
def get_audio_drama_adaptation_rules() -> str:
    ...
    return textwrap.dedent(
        ...
        10. 对白默认按短语连贯表达，像人物正在回应眼前的人。表演提示优先交代试探、回避、阻止或决定等说话目的，让语调随语意自然变化。不要把普通对白拆成逐字发音、固定重音或逐句收尾动作。
        ...
        productionNote 优先简短说明说话目的，例如“边翻包边顺口回答熟人”。普通对白无需特殊表达时留空，不必每句安排重音、放慢、停顿、压低或句尾收住。仅原文情节或用户明确要求时写具体发声变化；不能只凭心理标签添加这些动作，也不堆叠多个动作。省略号和断句须有真实的迟疑、中断或语意依据，不用来装饰普通对白。明确需要笑声、哭声、喊叫或特殊文体时仍应保留相应表达。
...
def get_audio_drama_script_prompt() -> str:
    ...
    return "\n\n".join([
        ...
        "每条人物声/旁白必须有 performanceCue：beatId 引用本场节拍，intent 写本句说话目的，delivery 默认留空，仅有情节或用户依据的特殊表达时补充，不重复 intent、共同基调或 productionNote。respondsTo 引用本场 lines 中此前正在回应的台词序号（从1开始，含音效行计数，不能指向音效或未来；没有则 null）。turningPoint 默认 false，仅原文或用户明确要求突变时为 true，evidence 必须逐字引用其直接依据。普通接话和标点不构成转折。音效/BGM 的 performanceCue 为 null。",
        ...
        "audioEvents 必须给出 timing、type、content、volume_db；type 只能是 sfx、amb、bgm、reverb、break。环境声通常 -28dB，背景音 -24dB，前景动作约 -12dB，确保不遮挡台词。必要的演员意图和特殊发声写 productionNote；普通对白按语意连贯说出。",
