...
`音效行 / 时间线 → SoundLibraryPanel → POST /sound-library/recommendations → SoundRecommendationService → 指定 LLM`。
推荐服务读取章节原文、目标提示、邻近台词和库内素材元数据，返回最多三个候选及原因；不生成音频、不改动台本或编排。用户选用后调用现有绑定或插入服务。全局 `/#/sound-library` 提供搜索、试听、导入。

仅允许 `qwen3.8-27b` / `kimi-k3`，按已启用且声明支持型号的服务商路由，不以账户失败为由切换模型。单次 completion 无自动重试；内容指纹缓存避免重复请求。模型只能返回候选目录内的 ID，后端再去重和校验；未知 ID 不进入选用按钮。推荐基于名称、标签与时长，听感由用户试听判断。

验证包含原文上下文、跨章节拒绝、禁用模型、虚构 ID、缓存失效、额度失败无重试、只读推荐以及素材副本保留。真实 qwen3.8-27b 请求已返回「窗边雨声」。推荐新增于前一轮工作台整合之后，属于独立的受控模型调用。
