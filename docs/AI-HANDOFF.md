...
3. 当前版本列表支持播放与删除，但还没有“设为最终采用版本”字段；导出仍默认使用 `line.audio_path` 原音频。
...
1. 增加 `active_audio_variant_id` 或同等机制，让用户把某个速度版本设为最终采用版本，并让导出/整章连播读取该版本。
2. 在重新生成台词原音频时提示旧版本来自上一 take，允许保留、归档或批量删除。
3. 为音频版本 API 增加路由级 TestClient 测试和异常场景测试（缺文件、非法速度、重复删除）。
4. 为音色试听抽取可复用 composable，覆盖人物草稿、人物档案和逐句音色下拉框，避免播放器状态代码重复。
5. 如用户要求全音色试听，为无 `reference_path` 音色增加后台预览生成队列，而不是在 GET 请求里同步调用云端 TTS。
6. 优化首页女歌姬素材体积，并复测 1600×900、1366×768、390×844 三个视口。
7. 处理任何新改动后运行全套验证并按用户偏好提交推送。
