...
- 新台本生成/返修使用 `DirectedDramaScript`，在原有 LLM 调用中同时生成场景任务、基调、人物目标/关系、顺序阶段，以及逐句 `performanceCue` 的意图、接话引用和转折依据；旧草稿仍由 `DramaScript` 兼容读取。项目配音背景和基调同时传入生成、审核、返修，不增加逐句 LLM 调用。
- 结构校验拒绝悬空/逆序阶段、缺失人物或提示、指向未来/音效的接话引用。独立审查检查语义；程序额外核对转折引用是否存在，不能把引用存在当作语义正确。当前有效设计只在引用核对通过的标记处放行强度突增。
- SQLite migration 8 为 `chapters` 增加可空 `performance_plan`。确认提交时把场景与提示绑定到真实台词 ID、保存源指纹；同名场景互相隔离。正文、角色、顺序、标签或指导变化时停止使用旧设计；状态、音频版本变化不影响设计。追加提交保留原场景且延续行序号。同步扩展 `ChapterEntity`，避免新增 ORM 字段导致章节创建/读取报错。
- TTS 只收到当前人物/阶段的表演信息，不包含后续高潮指导。原生指导新增保守的加权字符预算，整字段省略时记录原因，超长本句指导在调用前失败；这不是厂商 tokenizer 的精确计数。CosyVoice 等模型仍按既有能力适配。请求记录保存采用的设计与最终请求。
- 台本确认卡和逐句输入预览新增“整场表演设计”。清除前端 18%/45字/连续旁白自动告警，改为描述性统计；补上数据校验层的默认微弱强度与英文空格保留，明确给定的强度不再被旁白类型强制改为中等。
- 验证：165 项后端、23 项前端及完整 `./scripts/verify.sh` 通过。新增测试覆盖生成/返修调用数、模型输入、前后阶段隔离、真实 worker 记录、设计过期、引用校验、重复场景名、追加与幂等提交、迁移保留旧章节。没有真实模型 API 试验，没有修改现有项目数据库或录音；本次未完成浏览器视觉验收（Tabbit 连接尝试未取得可用会话）。
- 产品说明更新在 [配音设定与排查](speech-direction.md)。旧项目不会自动调用模型补设计，需在用户主动生成/返修台本时产出。

## 2026-09-11 旁白、上下文、发音词表与生成记录

- 取消共享规则、旧拆分入口与改编返修中的固定旁白比例、单句字数和连续旁白禁令；程序统计改为描述性信息，审核判断叙事功能、重复与听觉因果。工作台快捷反馈改为“检查旁白”。
- 新增 `SpeechDirectionService`：读取项目背景/基调、已提交章节解析、当前连续场景的前两句/后一句与同人物前次发言。确定性约束只限制无依据的强度突增，不抬高弱情绪；明确转折可放行。按模型能力发送原生上下文、精简指导或参数，背景不混进朗读正文。
- 项目级发音词表：Qwen-Audio 3.0 内置 HTTP 请求使用 `hot_fix.pronunciation`；其他模型可使用替代读法，未应用拼音明确提示。原台词与字幕保留，替换只执行一次；英文正文保留词间空格。
- SQLite migration 7 增加 `project_speech_profiles`、`tts_generations`。worker 为每次执行记录输入快照，传输层记录最终 HTTP 体/SDK 参数及可用返回，关联音频版本；准备和调用失败也留痕。独立 recorder Session 避免跨线程复用记录会话；不存音频本体、认证头或已知凭证。删除台词保留记录，删除项目清理。
- 工作台顶部新增“配音设定”，逐句新增“生成输入与记录”；预览只读已保存台词且不调用模型。静态 Demo 隐藏后端入口。详见[使用说明与能力边界](speech-direction.md)。
- 验证：151 项后端、21 项前端与完整 `./scripts/verify.sh` 通过；覆盖模拟 HTTP/SDK 适配、worker 成功/失败/准备错误、预览与设置 API、迁移与项目删除。没有调用真实模型 API，没有修改用户现有项目数据库或录音。
- 视觉验收未完成：Tabbit 普通与获批本地连接均返回 `BROWSER_RUNTIME_UNAVAILABLE`。未切换浏览器后端或改动用户浏览器配置；前端构建通过不代表视觉验收通过。后续需浏览器检查两个新对话框；实际读音和表演改善仍需另行授权的 API 试听对照。

## 2026-09-11 情绪提示与 TTS 适配

- 共享台本规则区分心理倾向与实际外露强度，日常对白默认低强度；补充题材/场景指导。审查与返修现在收到用户原始要求和本轮反馈。
- 原生 TTS 使用具体交谈、重音和句尾指导，中等及以下不直接重复分类情绪名。CosyVoice 按实际音色生成精简或结构化指令，固定情绪模式的微弱/稍弱映射为 neutral。
- Edge/CosyVoice 韵律映射跳过否定分句，情绪强度不再当作音量；Edge 默认参数变化减弱。Qwen3 Instruct 明确保持官方 optimize_instructions=false 默认，用户可主动开启。
- 未修改现有数据库模板或录音，最新共享规则在运行时注入；重启后端后，新生成使用新规则。没有真实云端或盲听验证，不声称音质已改善。
- 验证：136 项后端 + 21 项前端测试、完整 verify 通过。依据、修正与剩余优化见[提示词与 TTS 审核](reviews/2026-09-11-prompt-tts-review.md)。

## 2026-09-07 音效与 BGM 倍速

- 时间线片段新增 `playback_rate`，范围 0.5–2；SQLite migration 6 给旧片段补默认 1.0，升级前本地数据库备份保存在配置目录的 `backups/`。人物声仍使用配音工作台的处理版本，不在这里改倍速。
- 编辑片段新增倍速数字输入、0.5/0.75/1/1.25/1.5/2 倍按钮及素材试听。变化按旧倍速/新倍速同比调整片段时长、淡入淡出，保留起点；单次时长和循环判定按倍速计算。恢复原长恢复为当前倍速的一次完整播放时长。
- 渲染采用 FFmpeg atempo 保持音调；需要循环时先循环源，再变速、裁剪、混音。渲染指纹/清单记录倍速，修改后旧成片失效，刷新保留倍速和调整。延时后重建连续采样时间戳，修复单片段有起点延迟时输出时长被截短的问题。原音频不改写。
- 浏览器素材试听使用 playbackRate；它用于对比倍速，裁剪、循环、淡入淡出以最终混音为准。
- 验证：127 后端 + 21 前端测试、完整 verify 通过；额外渲染测试覆盖倍速加循环。440 Hz 信号以 0.5 倍渲染后仍约 440 Hz，1 秒片段变为 2 秒且起点 150 ms 保留；迁移保留旧片段位置/时长。当前项目 13 / 章节 9 第 3 行开门声设为 0.75 倍，415 ms → 553 ms，保留用户设置的 200 ms 起点；浏览器试听 readyState=4、playbackRate=0.75，保存后刷新和成片渲染使用该倍速。

## 2026-09-07 叠加混音与时间线便捷编辑

- 自动编排改为只有人物声/旁白推进对白时间；普通 BGM / SFX 在其台本位置叠加，手动移动过的素材也不推迟人声。源指纹使用 `overlay_v2`，旧编排会提示刷新。默认新编排 BGM 为 -18 dB，SFX 为 -6 dB；手动混音保留。
- BGM / SFX 可拖动左右边界延长至超过源时长，渲染使用 FFmpeg `-stream_loop -1` 重复播放并按片段时长截断，不拉伸或改写原文件。人物声/旁白仍限制在源时长内，全章最长 4 小时。手动循环长度在刷新和素材替换后保留，清单标记 `is_looping`。循环是重复录音，不保证任意一次性音效接缝天然连续。
- 拖动整段移动，边缘吸附到其他片段的起止点，竖线提示，Alt 临时关闭。编辑框选择参考片段后可「起点对齐 / 终点对齐 / 接在后面 / 覆盖参考片段」；素材还有「铺满人声 / 延长 5 或 10 秒 / 恢复原长」。快捷设置点击保存后生效；取消拖动恢复原位置，缩短自动约束淡入淡出。
- 短片段固定高度并省略溢出文字，避免竖向长条；同轨重叠片段分行显示。拖动至可视区边缘时横向滚动。
- 验证：125 项后端、20 项前端测试及完整 verify 通过；测试实际采样值证明循环后的 BGM 与对白叠加可听。浏览器已拖动雨声右边界从 10.866 秒延长到 15.9 秒、整段移动到 2 秒，再通过参考首句起点对齐和恢复原长回到原编排。当前项目 13 / 章节 9 人声从 0 秒开始；BGM 已铺满人声 0–72.006 秒并设置 -18 dB、淡入 800 ms、淡出 2500 ms，重新混音。之前 6 条未配素材依旧列出并跳过。

## 2026-09-07 时间线刷新、阶段成片与删除台词

- 修复任一 `is_user_edited` 片段导致刷新永久停留在 stale 的问题。默认刷新保留手动片段的 ID、位置、音量、静音和淡入淡出，更新所选音频；完整片段随源音频变长，裁剪片段限制在新源时长内。新生成的台词加入自动编排；`overwrite_manual=true` 仍用于显式放弃手动调整。
- 允许 `missing_audio` 时间线渲染已有片段，空时间线仍拒绝渲染。工作台点击渲染先刷新，界面列出跳过行并可跳回台词；输出标为「阶段成片（部分音频）」。渲染清单 schema 2 保存缺失行及源指纹，补齐音频后旧成片过期；阶段成片可再次读取、播放和下载。
- 逐句工作台增加「删除台词」及含目标文本的确认框。删除时备份行、任务及关联编排至本地 `deleted_lines/`，在同一数据库事务中移除行、任务、对应片段并重排行号；已有源音频及资产文件不删除。共享资产解除被删行的外键，附着音效按已编排位置保留；排队/生成中的句子禁止删除。删除记录无恢复 UI，供本地恢复排查使用。
- 验证：124 项后端、16 项前端测试及完整 `./scripts/verify.sh` 通过。真实项目 13 / 章节 9 从 stale 成功刷新为 42 个片段，6 条音效/BGM 尚缺音频，成功渲染 171.651 秒 WAV，浏览器音频 readyState=4。原手动片段参数一致，已有 203 个源音频 SHA256 均未改变。真实台词删除框验证后取消，实际删除及外键、共享音频、锚点、繁忙任务保护使用隔离测试数据验证。
...
- 删除台词、章节、项目或替换章节台词时会清理轨道、片段和音频资产；SQLite 连接已启用外键约束。手工编辑片段在刷新时保留调整并更新音频，只有显式 `overwrite_manual=true` 才放弃手动调整。
