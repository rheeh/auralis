<template>
  <div>
    ...
    <div class="voice-table-shell">
      ...
      <el-table-column label="播放" width="96" align="center">
      ...
      <!-- 描述改为 tag 展示 -->
      <el-table-column prop="description" label="标签" min-width="230">
        <template #default="{ row }">
          <div class="tags-wrap">
            <el-tag
      ...
      <el-table-column label="参考音频/路径" min-width="190" align="left">
        <template #default="{ row }">
          <el-tooltip :content="referenceLabel(row)" placement="top">
            <span class="path-ellipsis">{{ referenceLabel(row) }}</span>
          </el-tooltip>
        </template>
      </el-table-column>

      <el-table-column label="创建时间" width="156" class-name="date-cell">
        <template #default="{ row }">{{ formatDateTime(row.created_at) }}</template>
      </el-table-column>
      <el-table-column label="更新时间" width="156" class-name="date-cell">
        <template #default="{ row }">{{ formatDateTime(row.updated_at) }}</template>
      </el-table-column>

      <el-table-column label="操作" min-width="276" align="center">
  <template #default="{ row }">
...
const isSeedingPresets = ref(false)
...
function togglePlay(row) {
  ...
  const key = isPathOnly ? referencePath : row.id

  if (currentPath.value === key) {
    if (isPlaying.value) {
      audioPlayer.pause()
    } else {
      audioPlayer.play().catch(() => ElMessage.error('无法播放该音频文件，请检查参考音频是否存在'))
    }
    return
  }
...
async function handleSeedPresets(overwrite = false) {
  ...
  try {
    ...
    ElMessage.success(`常见音色已生成：新增/更新 ${data.success_count || 0} 个，跳过 ${data.skipped_count || 0} 个${failedText}`)
    await loadVoices()
  } catch (e) {
    console.error(e)
...
function getDefaultVoiceDir() {
  // 默认为用户目录下的 Auralis/voices
  const userHome = native?.getUserHome?.() || ''
  return userHome ? `${userHome}/Auralis/voices` : ''
}
...
.voice-table :deep(.date-cell .cell) {
  white-space: nowrap;
}
.tags-wrap {
  display: flex;
