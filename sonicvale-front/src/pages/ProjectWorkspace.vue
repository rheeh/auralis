    ...
    <section class="workspace-grid">
      <aside class="conversation-panel">
        <header class="assistant-head">
          <span class="assistant-avatar"><el-icon><MagicStick /></el-icon></span>
          <div><strong>Auralis 制作助手</strong><small>{{ snapshot ? stageDescription : '把小说发给我，我们从角色开始。' }}</small></div>
          <el-button v-if="snapshot" text size="small" @click="startNew">新改编</el-button>
        </header>

        <div class="conversation-scroll">
          <section v-if="snapshot" class="assistant-mission">
            <div><span>当前任务</span><el-tag size="small" effect="plain">{{ stageLabel }}</el-tag></div>
        ...
        <div v-if="!snapshot" class="source-composer">
          ...
          <div class="composer-footer"><span>{{ sourceChars }} 字</span><el-button type="primary" :loading="submitting" :disabled="!sourceChars" @click="createSession">发送给 AI</el-button></div>
        </div>
        ...
        <div v-else class="session-composer">
          <div class="assistant-actions">
            ...
            <template v-else-if="snapshot.current_stage==='completed'">
              ...
              <el-button size="small" @click="playAllFromAssistant">一键播放</el-button>
              <el-button type="primary" size="small" @click="generateFromAssistant">生成缺失试听</el-button>
            </template>
          ...
          <el-input
            ...
            :rows="3"
            resize="none"
            :disabled="!canRevise || actionBusy"
            :placeholder="canRevise ? '直接告诉 AI 要修改的人物、台词、旁白或音效……' : '当前阶段不需要输入；可使用上方快捷操作。'"
            @keydown.enter.exact.prevent="sendRevision"
          />
          <div class="composer-footer">
            <el-button :icon="Refresh" :loading="refreshing" @click="refresh">刷新</el-button>
            <el-button type="primary" :disabled="!canRevise || !feedback.trim() || actionBusy" :loading="actionBusy" @click="sendRevision">发送给制作助手</el-button>
          </div>
        </div>
      </aside>
      ...
      <main class="result-panel">
        ...
        <ScriptDraftConfirmCard
          v-else-if="snapshot.current_stage === 'awaiting_script_confirmation'"
          :script="snapshot.script_draft"
          :revision="snapshot.draft_revision"
          :loading="actionBusy"
          confirm-label="台本满意，进入逐句制作"
...
const actionBusy = computed(() => submitting.value || transitioning.value)
...
const isGenerating = computed(() => ['created','parsing','generating_script','committing'].includes(snapshot.value?.current_stage))
...
function startNew() { clearTimeout(pollTimer); snapshot.value=null;messages.value=[];feedback.value='';draft.title='';draft.source_text='';resultView.value='output' }
...
async function sendRevision() {
  if (!canRevise.value || !feedback.value.trim()) return
...
async function submitRevision(message) {
  ...
  try {
    ...
    feedback.value='';transitioning.value=true;transitionFromStage.value=snapshot.value.current_stage;transitionStartedAt=Date.now();await refreshHistory();schedulePoll()
  }catch(error){ElMessage.error(apiError(error,'提交修改意见失败'))}finally{submitting.value=false}
}
...
function schedulePoll() {
  ...
  if (isGenerating.value || autoCommit.value || transitioning.value) pollTimer=setTimeout(async()=>{await refresh()},1000)
}
function playAllFromAssistant(){resultView.value='output';setTimeout(()=>productionRef.value?.togglePlayAll?.(),0)}
function generateFromAssistant(){resultView.value='output';setTimeout(()=>productionRef.value?.generateAudio?.(),0)}
function apiError(error,fallback){return error?.response?.data?.message||error?.message||fallback}
</script>

<style scoped>
.project-workspace{height:100%;min-height:0}.workspace-meta,.assistant-head,.composer-footer,.result-head,.stage-dots{display:flex;align-items:center;gap:10px}.result-head,.composer-footer{justify-content:space-between}.eyebrow,.result-head h2{margin:0}.eyebrow{margin-bottom:3px;color:var(--el-color-primary);font-size:11px;text-transform:uppercase}.workspace-grid{display:grid;grid-template-columns:minmax(320px,380px) minmax(0,1fr);gap:10px;height:100%;min-height:0}.conversation-panel,.result-panel{min-height:0;border:1px solid var(--el-border-color-lighter);border-radius:14px;background:var(--el-bg-color);box-shadow:var(--auralis-shadow-sm);overflow:hidden}.conversation-panel{display:grid;grid-template-rows:auto minmax(0,1fr) auto}.assistant-head{padding:10px 12px;border-bottom:1px solid var(--el-border-color-lighter);background:linear-gradient(135deg,color-mix(in srgb,#69e9ef 10%,var(--el-bg-color)),color-mix(in srgb,#e58ad7 9%,var(--el-bg-color)))}.assistant-head>div{min-width:0;flex:1}.assistant-avatar{display:grid;place-items:center;width:32px;height:32px;border-radius:10px;color:#15162f;background:linear-gradient(135deg,#80eff2,#b5b2ff 55%,#ff9bd5)}.assistant-head strong,.assistant-head small{display:block}.assistant-head small{margin-top:2px;color:var(--el-text-color-secondary);font-size:11px;line-height:1.35}.conversation-scroll{min-height:0;overflow:auto;padding:12px}.welcome-message{padding:13px;border-radius:11px;background:var(--el-fill-color-light)}.welcome-message p{margin:6px 0 0;color:var(--el-text-color-secondary);font-size:12px;line-height:1.6}.source-composer,.session-composer{display:grid;gap:8px;padding:10px;border-top:1px solid var(--el-border-color-lighter);background:var(--el-fill-color-extra-light)}.composer-footer span{color:var(--el-text-color-secondary);font-size:11px}.result-panel{display:flex;flex-direction:column}.result-head{flex:0 0 auto;padding:10px 14px;border-bottom:1px solid var(--el-border-color-lighter)}.result-head h2{font-size:20px}.stage-dots{flex-wrap:wrap}.stage-dots span{padding:4px 8px;border-radius:999px;color:var(--el-text-color-secondary);background:var(--el-fill-color-light);font-size:11px}.stage-dots span.active{color:#18212d;background:linear-gradient(135deg,#8feeee,#e8a3dd)}.stage-dots span.done{color:var(--el-color-success);background:var(--el-color-success-light-9)}.result-panel>:not(.result-head){margin:14px;overflow:auto}.result-empty{display:grid;place-items:center;align-content:center;min-height:360px;text-align:center;color:var(--el-text-color-secondary)}.result-empty .el-icon{font-size:54px}.result-empty h3{margin:16px 0 6px;color:var(--el-text-color-primary)}.result-empty p{max-width:460px;margin:0;line-height:1.7}.processing-state{display:grid;grid-template-columns:minmax(0,1fr) 260px;gap:20px;align-items:center}.processing-state p,.commit-state p{color:var(--el-text-color-secondary);line-height:1.6}.commit-state{display:flex;align-items:center;justify-content:space-between;gap:16px;padding:18px!important;border:1px solid color-mix(in srgb,var(--el-color-success) 35%,var(--el-border-color));border-radius:12px;background:color-mix(in srgb,var(--el-color-success) 5%,var(--el-bg-color))}@media(max-width:1050px){.workspace-grid{grid-template-columns:minmax(290px,340px) minmax(0,1fr)}.stage-dots{display:none}}@media(max-width:760px){.project-workspace{height:auto}.workspace-grid{grid-template-columns:1fr}.conversation-panel{min-height:620px}.result-panel{min-height:600px}.processing-state{grid-template-columns:1fr}}
