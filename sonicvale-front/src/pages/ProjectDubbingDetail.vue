    ...
    <div class="page-wrap">
        ...
        <div class="header">
            <div class="title-side">
                ...
                <el-button text type="primary" @click="$router.push(`/projects/${projectId}/overview`)">
                    <el-icon>
                        <DocumentChecked />
                    </el-icon> 项目总览
                </el-button>
        ...
        <section class="agent-workspace">
            <aside class="agent-chat-panel">
                ...
                <div class="agent-thread">
                    <div
                        ...
                        :key="message.id"
                        class="agent-bubble"
                        :class="[message.role, { muted: message.muted }]"
                    >
                ...
                <div class="agent-command">
                    <el-input
                        ...
                        @keydown.enter.exact.prevent="runAgentCommand"
                        placeholder="向 Agent 描述下一步，例如：把这一章拆成更悬疑的广播剧台本"
                    />
                    <div class="agent-command-actions">
                        <el-button type="primary" :disabled="!agentPrompt.trim()" @click="runAgentCommand">
                            发送
                        </el-button>
                        <el-button type="primary" :disabled="!currentChapterContent" @click="splitByLLM">
                            <el-icon>
                                <MagicStick />
                            </el-icon>
                            拆台词
                        </el-button>
                        <el-button type="success" plain @click="generateAll">
                            <el-icon>
                                <Headset />
                            </el-icon>
                            生成配音
                ...
                <div class="agent-quick-actions">
                    ...
                    <el-button plain size="small" @click="openQueue = true">
                        <el-icon>
                            <Headset />
                        </el-icon>
                        队列
                    </el-button>
                    <el-button plain size="small" @click="openProjectSettings">
                        <el-icon>
                            <Setting />
                        </el-icon>
                        设置
            ...
            <main class="canvas-shell">
                <div class="canvas-toolbar">
                    <div>
                        <strong>无限画布</strong>
                        <span>章节、台词、角色和配音轨道</span>
                    </div>
                    <div class="canvas-status">
                        <el-tag effect="plain">台词 {{ lines.length }}</el-tag>
                        <el-tag effect="plain" type="success">已生成 {{ generationStats.done }}</el-tag>
                        <el-tag effect="plain" type="warning">剩余 {{ queue_rest_size }}</el-tag>
                        <div class="canvas-view-controls" aria-label="画布视图控制">
                            <el-button size="small" :disabled="canvasScale <= 0.8" @click="zoomCanvas(-0.1)">-</el-button>
                            <span class="canvas-zoom-value">{{ Math.round(canvasScale * 100) }}%</span>
                            <el-button size="small" :disabled="canvasScale >= 1.4" @click="zoomCanvas(0.1)">+</el-button>
                            <el-button size="small" plain @click="resetCanvasView">重置</el-button>
                        </div>
                    </div>
                </div>
                <div
                    ref="canvasScrollRef"
                    class="canvas-scroll"
                    :class="{ 'is-panning': isCanvasPanning, 'is-space-panning': isSpacePanning }"
                    ...
                    @scroll="saveCanvasView"
                >
                    <section class="canvas-board" :style="canvasBoardStyle">
                        <section class="canvas-node-map" aria-label="制作画布节点">
                            <button
                        ...
                        <section class="canvas-insight-grid">
                            <article class="canvas-insight">
                                <strong>角色声线</strong>
                                <span>{{ roles.length }} 个角色，{{ missingVoiceLineCount }} 句缺音色</span>
                                <el-button size="small" plain @click="activeTab = 'roles'">查看角色</el-button>
                            </article>
                            <article class="canvas-insight">
                                <strong>素材缺口</strong>
                                <span>{{ materialLineCount }} 条素材轨，{{ missingMaterialLineCount }} 条未绑定</span>
                                <el-button size="small" plain @click="$router.push('/media')">打开素材库</el-button>
                            </article>
                            <article class="canvas-insight">
                                <strong>导出状态</strong>
                                <span>{{ generationStats.done }} / {{ generationStats.total }} 条已完成</span>
                                <el-button size="small" type="success" plain @click="markAllAsCompleted">导出制作包</el-button>
                            </article>
                        </section>
                <!-- 未选择章节时显示提示 -->
                ...
                <template v-else>
                <!-- 章节正文 -->
                <el-card class="chapter-card canvas-panel">
        ...
        <!-- 右侧任务队列 -->
        <el-drawer v-model="openQueue" size="420px">
            <template #header>
                <div class="queue-drawer-header">
                    <span>任务队列</span>
                    <el-button text type="primary" :disabled="queue.length === 0" @click="clearQueue">清空</el-button>
                </div>
            </template>
            <el-empty v-if="queue.length === 0" description="暂无任务消息" />
            <el-timeline v-else>
...
import {
    Lock, Unlock, ArrowLeft, Setting, Headset, Menu, Plus, Search, Edit, Delete, Refresh, MagicStick, Document, DocumentChecked, CaretBottom, CaretRight, Upload, VideoPlay, VideoPause, Mute, Check,
...
const canvasPanState = ref({ pointerId: null, x: 0, y: 0, left: 0, top: 0 })
...
const transientQueueTitles = new Set(['已连接任务通道', '任务通道已断开', '心跳超时'])

function persistQueue() {
    try {
        const keep = queue.value.filter(q => !transientQueueTitles.has(q.title))
        localStorage.setItem(`queue_${projectId}`, JSON.stringify(keep))
    } catch { }
}
...
function addQueue(item) {
    ...
    if (item.persist !== false) persistQueue()
}
function restoreQueue() {
    try {
        const raw = localStorage.getItem(`queue_${projectId}`)
        if (raw) {
            queue.value = JSON.parse(raw).filter(q => !transientQueueTitles.has(q.title))
            persistQueue()
        }
    } catch { }
}
...
function startHeartbeat() {
    ...
    heartbeatTimer = setInterval(() => {
        ...
        heartbeatTimeout = setTimeout(() => {
            ...
            addQueue({ title: '心跳超时', meta: '触发重连', type: 'warning', persist: false });
...
function connectWS() {
    ...
    ws.onopen = () => {
        wsRetry = 0
        addQueue({ title: '已连接任务通道', meta: `项目 ${projectId}`, type: 'primary', persist: false })
        // 启动心跳
        startHeartbeat();
    ...
    ws.onmessage = (evt) => {
        try {
            ...
            if (msg.event === 'line_update') {
                ...
                if (msg.progress === 0 && !['processing', 'queued', 'skipped'].includes(msg.status)) {
    ...
    ws.onclose = () => {
        ...
        addQueue({ title: '任务通道已断开', meta: `将于 ${delay}ms 后重连`, type: 'warning', persist: false })
...
async function loadChapterDetail(chapterId) {
    if (!chapterId || Number.isNaN(Number(chapterId))) return
    ...
    const base = [
        {
            ...
            role: 'assistant',
            title: '当前项目',
            text: `${project.value?.name || '未命名项目'}，共 ${stats.value.chapterCount} 章、${stats.value.lineCount} 句台词。`,
        },
        currentChapter.value
            ? {
                ...
                role: 'user',
                title: '正在制作',
                text: currentChapter.value.title,
            }
            : {
                id: 'next',
                role: 'assistant',
                title: '下一步',
                text: '选择章节，或直接导入正文开始拆分台词。',
                muted: true,
            },
    ]
    return [...base, ...agentHistory.value.slice(-6)]
})

const speakableLineCount = computed(() => lines.value.filter(isSpeakableLine).length)
const materialLineCount = computed(() => lines.value.filter(line => !isSpeakableLine(line)).length)
const missingVoiceLineCount = computed(() => lines.value.filter(line => isSpeakableLine(line) && !getRoleVoiceId(line.role_id)).length)
const missingMaterialLineCount = computed(() => lines.value.filter(line => !isSpeakableLine(line) && !hasPlayableAudio(line)).length)
const canvasNodes = computed(() => [
    {
        key: 'chapter',
        index: '01',
        title: activeChapterId.value ? '章节已选定' : '选择章节',
        caption: currentChapter.value?.title || '从左侧上下文选一章',
        state: activeChapterId.value ? 'done' : 'pending',
    },
    {
        key: 'source',
        index: '02',
        title: currentChapterContent.value ? '正文已导入' : '导入正文',
        caption: currentChapterContent.value ? `${currentChapterContent.value.length} 字` : '粘贴小说原文或章节',
        state: currentChapterContent.value ? 'done' : 'pending',
    },
    {
        key: 'lines',
        index: '03',
        title: lines.value.length ? '台词已拆分' : '拆分台词',
        caption: lines.value.length ? `${speakableLineCount.value} 句朗读，${materialLineCount.value} 条素材` : 'LLM 生成旁白、对白、音效轨',
        state: lines.value.length ? 'done' : (currentChapterContent.value ? 'ready' : 'pending'),
    },
    {
        key: 'voices',
        index: '04',
        title: missingVoiceLineCount.value ? '补角色声线' : '声线就绪',
        caption: missingVoiceLineCount.value ? `${missingVoiceLineCount.value} 句缺音色` : `${roles.value.length} 个角色`,
        state: missingVoiceLineCount.value ? 'warning' : (lines.value.length ? 'done' : 'pending'),
    },
    {
        key: 'materials',
        index: '05',
        title: missingMaterialLineCount.value ? '补音效素材' : '素材轨就绪',
        caption: missingMaterialLineCount.value ? `${missingMaterialLineCount.value} 条音效/BGM 未绑定` : `${materialLineCount.value} 条素材轨`,
        state: missingMaterialLineCount.value ? 'warning' : (lines.value.length ? 'done' : 'pending'),
    },
    {
        key: 'generate',
        index: '06',
        title: generationStats.value.done >= generationStats.value.total && generationStats.value.total ? '配音完成' : '生成配音',
        caption: generationStats.value.total ? `${generationStats.value.done} / ${generationStats.value.total}` : '批量生成 TTS',
        state: generationStats.value.done >= generationStats.value.total && generationStats.value.total ? 'done' : (lines.value.length ? 'ready' : 'pending'),
    },
    {
        key: 'export',
        index: '07',
        title: '导出制作包',
        caption: '混音音频、字幕、制作表、清单',
        state: generationStats.value.done ? 'ready' : 'pending',
    },
])
...
function canvasViewKey() {
    return `auralis_canvas_view_${projectId}`
}

function saveCanvasView() {
    if (!canvasScrollRef.value) return
    try {
        localStorage.setItem(canvasViewKey(), JSON.stringify({
            scale: canvasScale.value,
            left: canvasScrollRef.value.scrollLeft,
            top: canvasScrollRef.value.scrollTop,
        }))
    } catch { }
}

async function restoreCanvasView() {
    try {
        const raw = localStorage.getItem(canvasViewKey())
        if (!raw) return
        const saved = JSON.parse(raw)
        if (Number.isFinite(saved.scale)) {
            canvasScale.value = Math.min(1.4, Math.max(0.8, Number(saved.scale)))
        }
        await nextTick()
        if (canvasScrollRef.value) {
            canvasScrollRef.value.scrollLeft = Number(saved.left) || 0
            canvasScrollRef.value.scrollTop = Number(saved.top) || 0
        }
    } catch { }
}

function zoomCanvas(delta) {
    const next = Math.min(1.4, Math.max(0.8, Number((canvasScale.value + delta).toFixed(2))))
...
function resetCanvasView() {
    canvasScale.value = 1
    if (canvasScrollRef.value) {
        canvasScrollRef.value.scrollLeft = 0
        canvasScrollRef.value.scrollTop = 0
    }
    saveCanvasView()
}
...
function isTextEditingTarget(target) {
    return Boolean(target?.closest?.('input,textarea,select,.el-input,.el-select,.el-textarea,[contenteditable="true"]'))
}

function isClickControlTarget(target) {
    return Boolean(target?.closest?.('button,a,.el-button'))
}

function beginCanvasPan(event) {
    const target = event.target
    const isBlankCanvas = target === canvasScrollRef.value || target?.classList?.contains('canvas-board')
    if (event.button !== 0 || isTextEditingTarget(target)) return
    if (!isSpacePanning.value && isClickControlTarget(target)) return
    if (!isBlankCanvas && !isSpacePanning.value) return
    isCanvasPanning.value = true
    canvasPanState.value = {
        pointerId: event.pointerId,
        x: event.clientX,
        y: event.clientY,
        left: canvasScrollRef.value?.scrollLeft || 0,
        top: canvasScrollRef.value?.scrollTop || 0,
    }
    canvasScrollRef.value?.setPointerCapture?.(event.pointerId)
    event.preventDefault()
}

function moveCanvasPan(event) {
    if (!isCanvasPanning.value || event.pointerId !== canvasPanState.value.pointerId) return
    const dx = event.clientX - canvasPanState.value.x
    const dy = event.clientY - canvasPanState.value.y
    if (canvasScrollRef.value) {
        canvasScrollRef.value.scrollLeft = canvasPanState.value.left - dx
        canvasScrollRef.value.scrollTop = canvasPanState.value.top - dy
    }
}
...
function endCanvasPan(event) {
    ...
    try {
        canvasScrollRef.value?.releasePointerCapture?.(event.pointerId)
    } catch { }
    isCanvasPanning.value = false
    canvasPanState.value = { pointerId: null, x: 0, y: 0, left: 0, top: 0 }
    saveCanvasView()
}
...
function pushAgentMessage(role, title, text, muted = false) {
    ...
    if (agentHistory.value.length > 12) agentHistory.value.splice(0, agentHistory.value.length - 12)
}
...
async function runAgentCommand() {
    ...
    if (!raw) return
    agentPrompt.value = ''
    pushAgentMessage('user', '指令', raw)

    const text = raw.toLowerCase()
    const has = (...words) => words.some(word => text.includes(word.toLowerCase()))

    if (has('配置', '模型', 'tts', 'api key', 'base url')) {
        pushAgentMessage('assistant', '已打开配置', '去配置中心检查 LLM / TTS / 路径设置。')
        router.push('/config')
        return
    }
    ...
    if (has('项目设置', '设置项目')) {
        openProjectSettings()
        pushAgentMessage('assistant', '已打开设置', '可以调整项目名称、模型、TTS 引擎和提示词。')
        return
    }

    if (has('队列', '消息')) {
        openQueue.value = true
        pushAgentMessage('assistant', '已打开队列', '可以查看批量任务、失败重试和生成进度。')
        return
    }

    if ((has('新建', '新增', '创建') && has('章节', '章')) || has('新章节')) {
        dialogNewChapter.value = true
        pushAgentMessage('assistant', '准备新建章节', '填写章节标题后即可继续导入正文。')
        return
    }

    if (!activeChapterId.value && chapters.value[0] && has('选择', '打开', '章节', '第一章', '首章')) {
        onSelectChapter(chapters.value[0])
        pushAgentMessage('assistant', '已选择章节', `当前章节：${chapters.value[0].title}`)
        return
    }

    if (has('导入', '粘贴', '正文', '原文')) {
        if (!activeChapterId.value) {
            if (chapters.value[0]) onSelectChapter(chapters.value[0])
            else {
                dialogNewChapter.value = true
                pushAgentMessage('assistant', '需要章节', '当前项目还没有章节，先新建章节再导入正文。')
                return
            }
        }
        const content = raw
            .replace(/^(请)?(帮我)?(导入|粘贴|保存|更新)?(当前|本)?(章节)?(正文|原文)?[：:\s]*/i, '')
            .trim()
        importText.value = content.length > 40 ? content : ''
        dialogImport.value = true
        pushAgentMessage('assistant', '准备导入正文', importText.value ? '已把指令中的正文预填到导入框，确认后保存。' : '导入框已打开，粘贴正文后保存。')
        return
    }
    ...
    if (has('拆', '拆分', '台词', 'llm')) {
        if (!currentChapterContent.value) {
            openImportDialog()
            pushAgentMessage('assistant', '缺少正文', '先导入本章节正文，再执行 LLM 拆分台词。')
            return
        }
        pushAgentMessage('assistant', '准备拆台词', '会调用当前项目配置的 LLM，并可能覆盖已有台词。')
        await splitByLLM()
        return
    }
    ...
    if (has('生成', '配音', '朗读')) {
        ...
        pushAgentMessage('assistant', '准备生成配音', '将按当前筛选范围批量生成可朗读台词。')
        generateAll()
        return
    }
    ...
    if (has('角色', '声线')) {
        ...
        pushAgentMessage('assistant', '已切到角色库', '可以绑定或更换角色默认音色。')
        return
    }

    if (has('音色')) {
        pushAgentMessage('assistant', '已打开音色库', '可以导入、试听或管理 Edge / 云端音色。')
        router.push('/voices')
        return
    }

    if (has('素材', '音效', 'bgm')) {
        pushAgentMessage('assistant', '已打开素材库', '去补音效、BGM 或替换占位素材。')
        router.push('/media')
        return
    }

    if (has('导出', '完成', '成片', '制作包')) {
        pushAgentMessage('assistant', '准备导出', '会检查当前章节音频并导出制作包。')
        await markAllAsCompleted()
        return
    }

    if (has('刷新', '同步')) {
        await Promise.all([loadChapters(), loadRoles(), activeChapterId.value ? loadLines() : Promise.resolve()])
        pushAgentMessage('assistant', '已同步', '项目、章节、角色和台词状态已刷新。')
        return
    }

    if (activeChapterId.value && raw.length > 60) {
        importText.value = raw
        dialogImport.value = true
        pushAgentMessage('assistant', '看起来像正文', '已把文本放入导入框，确认后保存到当前章节。')
        return
    }

    pushAgentMessage('assistant', '可执行指令', '可以说：导入正文、拆台词、生成配音、打开素材、打开音色、打开队列、导出制作包。', true)
}

function runCanvasNode(node) {
    if (node.key === 'chapter') {
        if (!activeChapterId.value && chapters.value[0]) onSelectChapter(chapters.value[0])
        else scrollToActiveChapter()
        return
    }
    if (node.key === 'source') {
        if (!activeChapterId.value && chapters.value[0]) onSelectChapter(chapters.value[0])
    ...
    if (node.key === 'lines') {
        activeTab.value = 'lines'
        if (!currentChapterContent.value) openImportDialog()
        else splitByLLM()
        return
    }
    if (node.key === 'voices') {
...
function statusText(s) {
    ...
    if (s === 'skipped') return '素材轨'
    return '未生成' // pending
}

function isSpeakableLine(row) {
    return row?.should_speak !== 0 && !['sfx', 'bgm'].includes(row?.track) && !['sfx', 'bgm'].includes(row?.line_type)
}

function trackLabel(row) {
    const track = row?.track || row?.line_type || 'voice'
    return {
        voice: '人物',
        narration: '旁白',
        sfx: '音效',
...
function canGenerate(row) {
    if (!isSpeakableLine(row)) return false
...
async function generateOne(row) {
    if (!isSpeakableLine(row)) {
        row.status = 'skipped'
        row.is_done = 1
        ElMessage.info('素材轨不进入 TTS，请导入或制作音效/BGM 文件')
        return
    }
    if (!canGenerate(row)) {
...
async function attachAssetAudio(row) {
    if (!row?.id) return
    if (!native?.pickAudio) {
        ElMessage.warning('当前环境不支持系统文件选择，请使用桌面端')
        return
    }

    const sourcePath = await native.pickAudio()
    if (!sourcePath) return

    try {
        const res = await lineAPI.attachAudioAsset(row.id, sourcePath)
        if (res?.code !== 200) {
            ElMessage.error(res?.message || '绑定素材失败')
            return
        }
        row.audio_path = res.data.audio_path
        row.status = 'done'
        row.is_done = 1
        bumpVer(row.id)
        ElMessage.success('素材音频已绑定')
    } catch (err) {
        console.error('绑定素材失败:', err)
...
function generateAll() {
    const todo = displayedLines.value.filter(l => isSpeakableLine(l) && canGenerate(l))
    if (!todo.length) {
        return ElMessage.info('无可生成的人物/旁白台词，或角色未绑定音色')
    }

    const skipped = displayedLines.value.filter(l => !isSpeakableLine(l)).length
    ElMessageBox.confirm(
        `此操作将会重新生成 ${todo.length} 条人物/旁白台词${skipped ? `，跳过 ${skipped} 条音效/BGM素材轨` : ''}，是否继续？`,
        '提示',
        {
...
function toggleVoicePlay(voiceId) {
    ...
    const src = voiceAPI.getVoiceAudioUrl(voiceId, Date.now())
    ...
    if (currentVoiceId.value === voiceId) {
        ...
        if (isPlaying.value) {
            ...
            audioPlayer.play().catch(() => ElMessage.error('无法播放音频，请检查音色参考文件是否存在'))
        }
        return
    }
...
async function markAllAsCompleted() {
    ...
    if (fail1 === 0 && fail2 === 0) {
        ...
        try {
            ...
            const excelOut = data.excel_path
            const manifestOut = data.manifest_path
            const exportedCount = data.exported_count || total
            const totalCount = data.total_count || total
            lastExportResult.value = data
            ...
            loading.setText(
                ...
                `- 制作表：${excelOut ? excelOut : '已导出'}\n` +
                `- 制作清单：${manifestOut ? manifestOut : '已导出'}\n` +
                `${msg1}；${msg2}`
            )

            // 友好提示
            if (data.missing_files && data.missing_files.length > 0) {
                ElMessage.warning(`导出完成（${exportedCount}/${totalCount}条），部分台词缺少音频；制作包已生成`)
            } else {
                ElMessage.success(`全部完成（共 ${exportedCount} 条）。音频、字幕、制作表和制作清单已导出`)
            }
        } catch (e) {
    ...
    try {
        const exportPath = lastExportResult.value?.manifest_path || lastExportResult.value?.audio_path
        const firstLineWithAudio = lines.value[0]
        const folderPath = exportPath
            ? getFolderFromPath(exportPath)
            : (firstLineWithAudio ? getFolderFromPath(firstLineWithAudio.audio_path) : '')
...
function waveSrc(row) {
    if (!row.audio_path) return ''
    return lineAPI.getLineAudioUrl(row.id, getVer(row.id))
}

function hasPlayableAudio(row) {
    return Boolean(row?.audio_path) && (row.status === 'done' || row.is_done === 1)
}
        ...
        cellRenderer: ({ rowData }) =>
            h('div', { style: 'display:grid;gap:6px;justify-items:center;' }, [
                h(
                    ...
                    {
                        size: 'small',
                        type: isSpeakableLine(rowData) ? 'primary' : 'warning',
                        effect: 'plain',
                    },
                    () => trackLabel(rowData)
                ),
                !isSpeakableLine(rowData)
                    ? h(ElText, { type: 'info', size: 'small' }, () => '不朗读')
                    : null,
            ]),
    },
    {
                ...
                [
                    isSpeakableLine(rowData)
                        ...
                        : h(
                            ...
                            {
                                size: 'small',
                                type: hasPlayableAudio(rowData) ? 'success' : 'warning',
                                plain: hasPlayableAudio(rowData),
                                onClick: () => attachAssetAudio(rowData),
                            },
                            () => (hasPlayableAudio(rowData) ? '替换素材' : '绑定素材')
                        ),
                    // ✅ 绿色的 is_done 开关
                    h(ElSwitch, {
...
// 3) 指定哪些列可拖（按你的 key 来）
;[
    'role_id', 'track', 'text_content', 'emotion_id', 'strength_id',
...
function restoreLastChapter() {
    const key = 'lastChapterMap';
    const map = JSON.parse(localStorage.getItem(key) || '{}');
    const last = Number(map[projectId]);

    if (last && chapters.value.find(c => Number(c.id) === last)) {
        // 只有当上次选择的章节仍然存在时才恢复
...
.agent-workspace {
    ...
    background:
        ...
        linear-gradient(135deg, rgba(120, 216, 223, 0.09), transparent 34%),
...
.agent-chat-panel {
    ...
    border-radius: 12px;
    background:
        linear-gradient(180deg, rgba(120, 216, 223, 0.13), transparent 220px),
        var(--el-bg-color);
    box-shadow: 0 18px 42px rgba(31, 42, 57, 0.08);
    overflow: hidden;
}
...
.agent-avatar {
    ...
    border-radius: 12px;
    color: #10202b;
    background: linear-gradient(135deg, #9bf0df, #82d8ff 72%, #ffab96);
    box-shadow: 0 14px 26px rgba(73, 196, 213, 0.22);
}
...
.agent-command-actions .el-button {
    flex: 1 1 120px;
}
...
.canvas-toolbar {
    ...
    justify-content: space-between;
    gap: 12px;
    min-width: min(760px, calc(100% - 430px));
    padding: 0 14px;
    border: 1px solid var(--el-border-color-light);
    border-radius: 10px;
    background: color-mix(in srgb, var(--el-bg-color) 94%, transparent);
    box-shadow: 0 16px 34px rgba(31, 42, 57, 0.08);
...
.canvas-scroll.is-space-panning {
    cursor: grab;
}
...
.canvas-insight {
    ...
    box-shadow: 0 12px 26px rgba(31, 42, 57, 0.07);
}
...
.canvas-node:focus-visible {
    ...
    box-shadow: 0 14px 30px rgba(39, 180, 195, 0.15);
    outline: none;
}

.canvas-node .node-index {
    display: inline-flex;
    align-items: center;
    width: max-content;
    min-height: 22px;
...
.canvas-node.is-done {
    ...
    background:
        linear-gradient(180deg, rgba(103, 194, 58, 0.1), transparent),
        var(--el-bg-color);
}
...
.canvas-node.is-ready {
    border-color: var(--el-color-primary-light-5);
    background:
        linear-gradient(180deg, rgba(36, 182, 198, 0.12), transparent),
        var(--el-bg-color);
}

.canvas-node.is-ready::after {
    background: var(--el-color-primary);
}

.canvas-node.is-warning {
    border-color: var(--el-color-warning-light-5);
    background:
        linear-gradient(180deg, rgba(230, 162, 60, 0.12), transparent),
        var(--el-bg-color);
}

.canvas-node.is-warning::after {
...
.el-tabs-box :deep(.el-tabs__header) {
    margin: 0;
    padding: 0 0 8px;
}

.el-tabs-box :deep(.el-tabs__nav-wrap::after) {
    display: none;
}

.el-tabs-box :deep(.el-tabs__content) {
    flex: 1 1 auto;
    min-height: 520px;
    position: relative;
    background: transparent;
