<template>
  <section class="script-card">
    ...
    <div v-if="script.adaptation_mode==='dialogue_lesson'" class="voice-cast" aria-label="双人声线配置">
      <span class="female"><b>知夏</b>年轻女声 · 清亮好奇</span>
      <span class="versus">×</span>
      <span class="male"><b>闻舟</b>成熟男声 · 沉稳耐心</span>
    </div>
    <div class="script-layout">
      <main class="segments">
        <article v-for="segment in script.segments || []" :key="segment.id" class="segment">
          ...
          <div v-for="(line,index) in segment.lines || []" :key="index" class="script-line" :class="[`track-${line.track}`,{'speaker-female':line.speaker==='知夏','speaker-male':line.speaker==='闻舟'}]">
            <strong>{{ line.speaker }}</strong><p>{{ line.text }}</p><span class="line-meta"><small v-if="line.emotion">{{ line.emotion }} · {{ line.strength || '中等' }}</small><small>{{ originLabel(line.content_origin) }}</small></span>
          </div>
        </article>
      </main>
