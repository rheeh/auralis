<template>
  <section class="chat-production">
    ...
    <template v-else>
      ...
      <div v-else-if="isGenerating" class="generating-card" role="status" aria-live="polite">
      ...
      <ScriptDraftConfirmCard
        v-if="snapshot.current_stage==='awaiting_script_confirmation'"
        :script="snapshot.script_draft" :revision="snapshot.draft_revision" :loading="isSubmitting" @confirm="confirmScript"
      />
      ...
      <div class="conversation-grid">
        ...
        <ChatComposer v-if="canRevise" :loading="isSubmitting" @send="sendFeedback" />
      </div>
    </template>
  </section>
</template>
...
const isGenerating = computed(()=>['created','parsing','generating_script'].includes(snapshot.value?.current_stage))
...
const stageLabel = computed(()=>({created:'准备解析',parsing:'正在解析原文',awaiting_role_confirmation:'等待确认角色',generating_script:'正在生成剧本',awaiting_script_confirmation:'等待确认剧本',script_draft_ready:'等待写入项目',committing:'正在写入项目',completed:'制作已完成',failed:'需要处理',cancelled:'已取消'}[snapshot.value?.current_stage]||'准备中'))
const stageDescription = computed(()=>({awaiting_role_confirmation:'角色草稿已准备好，需要你的确认。',awaiting_script_confirmation:'剧本草稿已准备好，可按场景检查。',script_draft_ready:'剧本已确认，正式项目数据尚未写入。',failed:'当前步骤未完成，可查看原因并重试。'}[snapshot.value?.current_stage]||'系统会保存每一步，页面刷新后仍可继续。'))
...
async function refreshHistory(){if(!snapshot.value)return;const response=await fetchChatHistory(snapshot.value.session_id,{limit:100});messages.value=response.code===200?response.data:[]}
...
function startPolling(){clearInterval(pollTimer);pollTimer=setInterval(async()=>{await refresh();if(!isGenerating.value&&!isSubmitting.value)clearInterval(pollTimer)},2200)}
