...
[▶ 开始体验 Auralis 静态 Demo](https://rheeh.github.io/auralis/)

静态 Demo 直接复用仓库中的 Vue 页面和正式生产组件，并以真实项目 `test4` 的《早读前的暗流》作为演示内容。小说原文、已解析人物、广播剧台本、音色绑定和现有生成音频均已预置，无需配置后端、模型服务或 API Key 即可体验完整流程。
...
- 内置 32 个 CC0 环境声与拟音素材，支持标签搜索、试听、用户上传以及绑定到 SFX/BGM 台词。
