...
已配置好本地环境后，可双击桌面 **Auralis.command**。它固定使用前端 `5176`、后端 `8200` 和仓库 `.local-data` 中的现有工程，直接打开项目列表；侧栏「音效库」可进入素材库。重复双击复用运行中的服务，终端提示就绪后可关闭窗口。每次启动只检查服务及项目/素材能否读取，不运行完整测试或调用模型。

快捷入口源码为 `scripts/Auralis.command`，可直接运行；macOS 用户可将它创建为桌面符号链接。首次安装仍按下方步骤准备依赖。端口被其他程序占用时会明确报错，不自动换端口；启动日志位于 `.local-data/launcher/`。
...
浏览器打开 [本地 Auralis](http://127.0.0.1:5176)，点击“开始创作”进入项目列表。
